...
component extends="testbox.system.BaseSpec" {
	...
	public function testCFQuery() {
		var rows = [ {"id":1,"title":"Dewey defeats Truman","live":0}, {"id":2,"title":"Man walks on Moon","live":1} ];
		var news = news = queryNew("id,title,live", "integer,varchar,boolean", rows);
		...
		";
local.params = {
	param1 = {list="yes", cfsqltype="cf_sql_integer", value="1,2"},
	param2 = {cfsqltype="cf_sql_bit", value="1"}}
;
local.options = {
	dbtype = "query"}
;
local.result = queryExecute(sql=local.sql,params=local.params,options=local.options);


			writeOutput("SELECT id, title
			FROM news
			WHERE id IN (");
			cfqueryparam( list=true, cfsqltype="cf_sql_integer", value="1,2" );

			writeOutput(")
			AND   live =");
			cfqueryparam( cfsqltype="cf_sql_bit", value=1 );
			$assert.isEqual(2, local.result.recordcount);
		}
